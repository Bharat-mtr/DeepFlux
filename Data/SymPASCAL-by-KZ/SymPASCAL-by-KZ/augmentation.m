...
for i=1:nImgs
   ...
   edge = single(gt.edge);  symmetry = single(gt.skeleton);
   scale = bwdist(edge);
   scale(symmetry==0)=0; scale = 2 * scale;
   if max(scale(:)) >=180
      rt =  180 / max(scale(:));
      edge = imresize(edge, rt);
      symmetry = imresize(symmetry, rt);
      edge = bwmorph(edge, 'thin', Inf);
      symmetry = bwmorph(symmetry, 'thin',Inf);
      im = imresize(im, rt);
   end
   for s = scales
